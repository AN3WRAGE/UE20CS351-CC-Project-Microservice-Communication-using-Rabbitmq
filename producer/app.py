...
c = mydb.cursor()
def create_table():
    c.execute('CREATE TABLE STUDENTS_INFO(name VARCHAR(50), SRN VARCHAR(15), section VARCHAR(1));')
# create_table()

app = Flask(__name__)

credentials = pika.PlainCredentials('guest', 'guest')
connection = pika.BlockingConnection(pika.ConnectionParameters('192.168.43.115', 5672, '/', credentials))
